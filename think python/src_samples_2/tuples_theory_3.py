...
counts = dict()
for line in fhand:
    line = line.translate(str.maketrans('', '', string.punctuation))  # this removes all the punctuation
    line = line.lower()
    words = line.split()
    for word in words:
        if word not in counts:
            # this translates as: for "word" variable in words sequence,
            # if word is not in dictionary, assign value 1 to the word, otherwise, increment 1
            counts[word] = 1
        else:
            counts[word] += 1
